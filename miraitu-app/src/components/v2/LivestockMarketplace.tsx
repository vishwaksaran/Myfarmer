    ...
    return (
        <section className="px-4 md:px-6 py-14 bg-gradient-to-b from-transparent via-primary/[0.02] to-transparent">
            <div className="mx-auto max-w-[1400px]">
                ...
                <div className="mb-8 flex gap-2 overflow-x-auto scrollbar-hide pb-1" data-no-auth>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6">
                    {filteredLivestock.map((item, index) => (
                        <div key={item.id} className={`skeuo-card-hover group flex flex-col overflow-hidden rounded-2xl md:rounded-3xl border border-white/10 opacity-0 animate-fade-in-up stagger-${index + 1}`}>
                            {/* Image Container */}
                            ...
                            <div className="flex flex-col p-3 md:p-5">
                                <div className="mb-2 md:mb-3">
                                    <h3 className="text-xs md:text-lg font-bold mb-0.5 md:mb-1 group-hover:text-primary transition-colors line-clamp-1">{item.name}</h3>
                                    <div className="flex flex-col md:flex-row md:items-center gap-0.5 md:gap-3 text-[10px] md:text-sm text-gray-500">
                                ...
                                <div className="mb-3 md:mb-4 space-y-1.5 md:space-y-2">
                                    ...
                                    <div className="h-1 md:h-1.5 w-full rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-1 text-[10px] md:text-xs text-gray-500">
                                        <span className="material-symbols-outlined text-[10px] md:text-xs text-primary">location_on</span>
                                ...
                                <div className="flex gap-1.5 md:gap-2">
                                    <button className="flex flex-1 items-center justify-center gap-1 md:gap-2 rounded-lg md:rounded-xl bg-primary py-2 md:py-2.5 text-[10px] md:text-xs font-bold text-white transition-all hover:brightness-110 active:scale-95 shadow-sm">
                                        <span className="material-symbols-outlined text-xs md:text-sm">call</span>
                                        {t('livestock.call')}
                                    </button>
                                    <button className="flex flex-1 items-center justify-center gap-1 md:gap-2 rounded-lg md:rounded-xl border border-green-200 bg-green-50 py-2 md:py-2.5 text-[10px] md:text-xs font-bold text-green-700 hover:bg-green-100 transition-all active:scale-95">
                                        <svg className="size-2.5 md:size-3.5" fill="currentColor" viewBox="0 0 24 24">
