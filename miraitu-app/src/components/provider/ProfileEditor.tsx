...
export default function ProfileEditor({ profile, onSaved }: ProfileEditorProps) {
    ...
    const handleAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file) return;
        if (!file.type.startsWith('image/')) { setAvatarMsg({ type: 'err', text: 'Please choose an image file' }); return; }
        if (file.size > 5 * 1024 * 1024) { setAvatarMsg({ type: 'err', text: 'Image must be under 5 MB' }); return; }
        setAvatarMsg(null);
        setUploading(true);
        ...
        e.target.value = '';
        if (url && !error) { setAvatarUrl(url); setAvatarMsg({ type: 'ok', text: 'Photo updated' }); onSaved?.(); }
        else setAvatarMsg({ type: 'err', text: error || 'Failed to upload image' });
    };
            ...
            <div className="bg-white dark:bg-[#1a231a] rounded-2xl border border-gray-100 dark:border-gray-800 p-5">
                <div className="flex items-center gap-4">
                    <div className="relative">
                        ...
                        >
                            <span className={`material-symbols-outlined text-sm ${uploading ? 'animate-spin' : ''}`}>{uploading ? 'progress_activity' : 'photo_camera'}</span>
                        </button>
                    ...
                    <div className="min-w-0">
                        ...
                        <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading} className="mt-1 text-xs font-bold text-primary disabled:opacity-50">
                            {uploading ? 'Uploading…' : 'Change photo'}
                        </button>
                        {avatarMsg && <p className={`text-[11px] font-semibold mt-0.5 ${avatarMsg.type === 'ok' ? 'text-green-600' : 'text-red-500'}`}>{avatarMsg.text}</p>}
                    </div>
                </div>
                <div className="mt-4">
                    <AvailabilityToggle
            ...
            <div className="bg-white dark:bg-[#1a231a] rounded-2xl border border-gray-100 dark:border-gray-800 p-5">
                ...
                <div className="space-y-2.5">
                    {DAYS.map(d => {
                        const h = hours[d.key];
                        return (
                            <div key={d.key} className="rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-800/30 p-3">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-sm font-bold text-gray-700 dark:text-gray-200">{d.label}</span>
