            ...
            <section className="px-4 md:px-6 pt-6 md:pt-10 pb-12">
                <div className="mx-auto max-w-[1280px]">
                    ...
                    <nav className="flex items-center gap-1 mb-6 text-xs md:text-sm">
                        ...
                        <span className="material-symbols-outlined text-gray-400 text-xs">chevron_right</span>
                        <span className="text-primary font-bold">{tp('Crop Costing')}</span>
                    </nav>
                    ...
                    <div className="mb-8">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <h1 className="text-2xl md:text-4xl font-black text-gray-900 dark:text-white">{tp('Crop Costing & ROI')}</h1>
                        </div>
                        <p className="text-sm md:text-base text-gray-500">{tp('Track input costs: Seeds, Fertilizers, Labor, and Irrigation to calculate ROI.')}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-8">
                        {Object.entries(cropPresets).map(([key, crop]) => (
                            ...
                            >
                                ...
                                <p className={`text-xs md:text-sm font-bold ${selectedCrop === key ? 'text-amber-600' : 'text-gray-600 dark:text-gray-400'}`}>{tp(crop.name)}</p>
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2">
                            <div className="skeuo-card rounded-2xl md:rounded-3xl p-4 md:p-8">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-lg font-black text-gray-900 dark:text-white">{tp('Input Costs (Per Acre)')}</h3>
                                    <button onClick={addItem} className="flex items-center gap-1 text-sm font-bold text-primary hover:text-primary/80">
                                        <span className="material-symbols-outlined text-sm">add_circle</span> {tp('Add Item')}
                                    </button>
                                </div>

                                {/* Header Row - hidden on mobile */}
                                <div className="hidden md:grid grid-cols-12 gap-2 text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 px-2">
                                    <div className="col-span-2">{tp('Category')}</div>
                                    <div className="col-span-3">{tp('Item Name')}</div>
                                    <div className="col-span-2">{tp('Qty')}</div>
                                    <div className="col-span-1">{tp('Unit')}</div>
                                    <div className="col-span-2">{tp('Rate (₹)')}</div>
                                    <div className="col-span-1 text-right">{tp('Cost')}</div>
                                            ...
                                            <div key={item.id} className="p-3 md:p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700">
                                                ...
                                                <div className="md:hidden space-y-2">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full text-sm font-bold bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border-none" placeholder={tp('Item name')}
                                                    />
                                                    <div className="grid grid-cols-3 gap-2">
                                                        <input type="number" value={item.qty} onChange={e => updateItem(item.id, 'qty', e.target.value)}
                                                            className="text-sm font-bold bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border-none" placeholder={tp('Qty')} />
                                                        <input value={item.unit} onChange={e => updateItem(item.id, 'unit', e.target.value)}
                                                            className="text-sm bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border-none" placeholder={tp('Unit')} />
                                                        <input type="number" value={item.rate} onChange={e => updateItem(item.id, 'rate', e.target.value)}
                                                            className="text-sm font-bold bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border-none" placeholder={tp('Rate ₹')} />
                                                    </div>
                                                ...
                                                <div className="hidden md:grid grid-cols-12 gap-2 items-center">
                                                    ...
                                                    <input value={item.name} onChange={e => updateItem(item.id, 'name', e.target.value)}
                                                        className="col-span-3 text-sm font-bold bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border-none" placeholder={tp('Item')} />
                                                    <input type="number" value={item.qty} onChange={e => updateItem(item.id, 'qty', e.target.value)}
                                                        className="col-span-2 text-sm font-bold bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border-none" placeholder={tp('Qty')} />
                                                    <input value={item.unit} onChange={e => updateItem(item.id, 'unit', e.target.value)}
                                                        className="col-span-1 text-xs bg-white dark:bg-gray-800 rounded-lg px-2 py-2 border-none" placeholder={tp('Unit')} />
                                                    <input type="number" value={item.rate} onChange={e => updateItem(item.id, 'rate', e.target.value)}
                                                        className="col-span-2 text-sm font-bold bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border-none" placeholder={tp('₹ Rate')} />
                                ...
                                <button onClick={addItem} className="w-full mt-4 py-3 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700 text-gray-400 font-bold text-sm hover:border-primary hover:text-primary transition-colors flex items-center justify-center gap-2">
                                    <span className="material-symbols-outlined text-sm">add</span> {tp('Add Cost Item')}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="skeuo-card rounded-2xl p-5 md:p-6">
                                <h4 className="font-bold text-gray-900 dark:text-white mb-4">{tp('Revenue Estimation')}</h4>
                                <div className="space-y-3">
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{tp('Land Size (Acres)')}</label>
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{tp('Expected Yield (Quintals/Acre)')}</label>
                                        <input type="number" value={expectedYield} onChange={e => setExpectedYield(e.target.value)}
                                            className="w-full skeuo-inset rounded-xl px-4 py-3 font-bold focus:ring-0 border-none" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{tp('Selling Price (₹ per Quintal)')}</label>
                            ...
                            <div className="skeuo-card rounded-2xl p-5 md:p-6 space-y-4">
                                <h4 className="font-bold text-gray-900 dark:text-white">{tp('Profit & Loss Summary')}</h4>

                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 rounded-xl bg-red-50 dark:bg-red-900/10 text-center">
                                        <p className="text-[10px] font-bold text-red-400 uppercase">{tp('Total Cost')}</p>
                                    ...
                                    <div className="p-3 rounded-xl bg-green-50 dark:bg-green-900/10 text-center">
                                        <p className="text-[10px] font-bold text-green-400 uppercase">{tp('Revenue')}</p>
                                ...
                                <div className={`p-4 rounded-xl text-center ${profit >= 0 ? 'bg-green-100 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-red-100 dark:bg-red-900/20 border border-red-200 dark:border-red-800'}`}>
                                    <p className="text-xs font-bold uppercase mb-1">{profit >= 0 ? tp('Net Profit') : tp('Net Loss')}</p>
                                    <p className={`text-2xl md:text-3xl font-black ${profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>{fmt(Math.abs(profit))}</p>
                                    <p className="text-xs text-gray-500 mt-1">ROI: <strong className={profit >= 0 ? 'text-green-600' : 'text-red-600'}>{roi.toFixed(1)}%</strong> | {tp('Cost/Qtl')}: {fmt(costPerQtl)}</p>
                                </div>
                            </div>

                            {/* Category Breakdown */}
                            <div className="skeuo-card rounded-2xl p-5 md:p-6">
                                <h4 className="font-bold text-gray-900 dark:text-white mb-4">{tp('Cost Breakdown')}</h4>
                                <div className="space-y-3">
                                    {Object.entries(categoryTotals).sort((a, b) => b[1] - a[1]).map(([cat, amount]) => (
                                        <div key={cat}>
                                            <div className="flex items-center justify-between text-sm mb-1">
                                                <span className="font-bold text-gray-700 dark:text-gray-300">{tp(cat)}</span>
