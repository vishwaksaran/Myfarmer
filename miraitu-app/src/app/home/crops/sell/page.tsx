            ...
            <div className="mx-auto max-w-[1280px]">
                ...
                <div className="mb-6 md:mb-10">
                    ...
                    <p className="text-base md:text-lg text-gray-500 max-w-2xl">
                        {tp('List your harvest and connect with buyers directly. No middlemen, better prices.')}
                    </p>
                </div>
                ...
                >
                    ...
                    <div className="min-w-0 flex-1">
                        <h2 className="text-lg md:text-xl font-bold mb-0.5">{tp('List Your Produce')}</h2>
                        <p className="text-white/80 text-sm md:text-base">{tp('Create a new listing and start getting buyer enquiries')}</p>
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8 md:mb-10">
                    {benefits.map((b) => (
                        <div key={b.title} className="p-4 md:p-5 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                            ...
                            <h3 className="font-bold text-sm md:text-base text-gray-900 dark:text-white mb-1">{tp(b.title)}</h3>
                            <p className="text-xs md:text-sm text-gray-500 leading-snug">{tp(b.desc)}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="mb-8 md:mb-10">
                    <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-4 md:mb-6">{tp('What you can sell')}</h2>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                        {cropTypes.map((crop) => (
                            ...
                            >
                                ...
                                <h3 className="font-bold text-sm md:text-base text-gray-900 dark:text-white">{tp(crop.name)}</h3>
                                <p className="text-xs text-gray-500 mt-1">{tp(crop.examples)}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 md:gap-4 mb-8 md:mb-10">
                    <div className="p-4 md:p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-center">
                        ...
                        <p className="text-xs md:text-sm text-gray-500 mt-1">{tp('Active Listings')}</p>
                    </div>
                    <div className="p-4 md:p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-center">
                        <p className="text-2xl md:text-3xl font-bold text-primary">8,000+</p>
                        <p className="text-xs md:text-sm text-gray-500 mt-1">{tp('Verified Buyers')}</p>
                    </div>
                    <div className="p-4 md:p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-center">
                        <p className="text-2xl md:text-3xl font-bold text-primary">15%</p>
                        <p className="text-xs md:text-sm text-gray-500 mt-1">{tp('Better Prices Avg.')}</p>
                    </div>
                </div>

                {/* How It Works */}
                <div className="mb-8 md:mb-10">
                    <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-4 md:mb-6">{tp('How it works')}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[
                            ...
                            <div key={s.step} className="flex items-start gap-4 p-4 md:p-5 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                                ...
                                <div>
                                    <h3 className="font-bold text-gray-900 dark:text-white mb-1">{tp(s.title)}</h3>
                                    <p className="text-sm text-gray-500">{tp(s.desc)}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl md:rounded-3xl p-6 md:p-8 text-white text-center">
                    <h2 className="text-xl md:text-2xl font-bold mb-2 md:mb-3">{tp('Ready to sell your harvest?')}</h2>
                    <p className="text-white/90 mb-4 md:mb-6 text-sm md:text-base">{tp('Create your first listing in under 5 minutes')}</p>
                    <Link
                        href="/home/crops/sell/list"
